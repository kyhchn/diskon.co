<x-app-layout>
    <form class="px-3 py-2" method="post" action="/reply/{{ $reply->id }}" class="mt-6 space-y-6">
        @method('PUT')
        @csrf

        <div>
            <x-input-label for="isi" :value="__('Isi')" />
            <x-text-input id="isi" name="isi" type="text" class="mt-1 block w-full" value="{{ $reply->isi }}"
